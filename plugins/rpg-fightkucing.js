...
conn.level = global.db.data.users[m.sender]
  ...
  if (pointPemain > pointLawan){
    ...
    m.reply(`*${conn.getName(m.sender)}* [${pointPemain * 10}] - [${pointLawan * 10}] *${conn.getName(lawan)}*\n\n*Pet🐈Kamu* (kucing ${global.db.data.users[m.sender].kucing}) MENANG melawan 🐈kucingnya *${conn.getName(lawan)}* (kucing ${global.db.data.users[lawan].kucing}) karena kucing🐈kamu ${alasanMenang[getRandom(0,alasanMenang.length-1)]}\n\nHadiah $${hadiah.toLocaleString()}\n+1 Tiketcoin`)
  }else if (pointPemain < pointLawan){
    let denda = (pointLawan - pointPemain) * 10
    global.db.data.users[m.sender].money -= denda
    global.db.data.users[m.sender].tiketcoin += 1
    m.reply(`*${conn.getName(m.sender)}* [${pointPemain * 10}] - [${pointLawan * 10}] *${conn.getName(lawan)}*\n\n*Pet🐈Kamu* (kucing ${global.db.data.users[m.sender].kucing}) KALAH melawan 🐈kucingnya *${conn.getName(lawan)}* (kucing ${global.db.data.users[lawan].kucing}) karena pet kamu ${alasanKalah[getRandom(0,alasanKalah.length-1)]}\n\nUang kamu berkurang $${denda.toLocaleString()}\n+1 Tiketcoin`)
  }else {
